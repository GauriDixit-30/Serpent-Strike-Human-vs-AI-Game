class Snake {
    ...
    setSpriteFrame(index) {
        ...
        if (index === 0) {  // head
            ...
            else if (prevSegment.x < segment.x && nextSegment.y > segment.y) {  // up and left
                segment.frameX = 4;
                segment.frameY = 0;
            } else if (prevSegment.y > segment.y && nextSegment.x > segment.x) {  // left and down
                segment.frameX = 3;
                segment.frameY = 0;
            } else if (prevSegment.x > segment.x && nextSegment.y < segment.y) {  // down and right
                segment.frameX = 3;
                segment.frameY = 1;
            } else if (prevSegment.y < segment.y && nextSegment.x < segment.x) {  // right and up
                segment.frameX = 4;
                segment.frameY = 1;
            }
            // bend clock wise
            else if (nextSegment.x < segment.x && prevSegment.y > segment.y) {  // right and down
                segment.frameX = 3;
                segment.frameY = 2;
            } else if (nextSegment.y < segment.y && prevSegment.x < segment.x) {  // down and left
                segment.frameX = 3;
                segment.frameY = 3;
            } else if (nextSegment.x > segment.x && prevSegment.y < segment.y) {  // left and up
                segment.frameX = 2;
                segment.frameY = 3;
            } else if (nextSegment.y > segment.y && prevSegment.x > segment.x) {  // up and right
                segment.frameX = 2;
                segment.frameY = 2;
            }
            else {
